...
class Node(object):
    ...
    def getrecord(self, *mpaths):
        ...
        LOGGER.debug('"%(terug)s" for getrecord%(mpaths)s', {
    ...
    def change(self, where, change):
        ...
        LOGGER.debug('"%(terug)s" for change(where=%(where)s,change=%(change)s)', {
    ...
    def delete(self, *mpaths):
        ...
        LOGGER.debug('"%(terug)s" for delete%(mpaths)s', {'terug': terug, 'mpaths': unicode(mpaths)})
    ...
    def get(self, *mpaths):
        ...
        LOGGER.debug('"%(terug)s" for get%(mpaths)s', {'terug': terug, 'mpaths': unicode(mpaths)})
        return terug
    ...
    def getloop(self, *mpaths):
        ...
        for terug in self._getloopcore(mpaths):
            LOGGER.debug('getloop %(mpaths)s returns "%(record)s".',
    ...
    def getloop_including_mpath(self, *mpaths):
        ...
        for terug in self._getloopcore_including_mpath(mpaths):
            LOGGER.debug('getloop %(mpaths)s returns "%(terug)s".', {'mpaths': mpaths, 'terug': terug})
            yield terug
    ...
    def put(self, *mpaths, **kwargs):
        ...
        for part in mpaths:
            ...
            for key, value in part.items():
                if value is None:
                    LOGGER.debug('"None" in put %(mpaths)s.', {'mpaths': unicode(mpaths)})
                    return False
                if not isinstance(key, basestring):
                ...
                if isinstance(value, list):
                    ...
                    if not value:
                        LOGGER.debug('Empty list in put %(mpaths)s.', {'mpaths': unicode(mpaths)})
                        return False
                else:
        ...
        LOGGER.debug('"True" for put %(mpaths)s', {'mpaths': unicode(mpaths)})
        return True
