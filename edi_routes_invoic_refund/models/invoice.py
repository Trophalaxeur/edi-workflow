...
class account_invoice(osv.Model, EDIMixin):
    ...
    @api.model
    def valid_for_edi_export_invoic_refund(self, record):
        if record.state != 'open':
            return False
        return True

    @api.multi
    def send_edi_export_invoic_refund(self, partner_id):
        valid_invoices = self.filtered(self.valid_for_edi_export_invoic_refund)
        invalid_invoices = [p for p in self if p not in valid_invoices]
        if invalid_invoices:
            raise except_orm(_('Invalid credit notes in selection!'), _('The following credit notes are invalid, please remove from selection. %s') % (map(lambda record: record.name, invalid_invoices)))

        for invoice in valid_invoices:
            content = invoice.edi_export_invoic_refund(invoice, None)
    ...
    @api.cr_uid_context
    def edi_export_invoic_refund(self, cr, uid, invoice, edi_struct=None, context=None):
        ...
        #else:
        edi_doc['LEVERINGSBON'] = invoice.origin[-8:]
