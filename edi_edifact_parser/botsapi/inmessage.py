...
class excel(csv):
    ...
    def initfromfile(self):
        ...
        try:
            ...
            txt = botslib.txtexc()
            LOGGER.error(_('Excel extraction failed, may not be an Excel file? Error:\n%(txt)s'),
                                    {'txt': txt})
...
class edifact(var):
    ...
    def handleconfirm(self, ta_fromfile, routedict, error):
        ...
        for nodeunb in self.getloop({'BOTSID': 'UNB'}):
            ...
            out.writeall()
            LOGGER.debug('Send edifact confirmation (CONTRL) route "%(route)s" fromchannel "%(fromchannel)s" frompartner "%(frompartner)s" topartner "%(topartner)s".',
...
class x12(var):
    ...
    def handleconfirm(self, ta_fromfile, routedict, error):
        ...
        for nodegs in self.getloop({'BOTSID': 'ISA'}, {'BOTSID': 'GS'}):
            ...
            LOGGER.debug('Send x12 confirmation (997) route "%(route)s" fromchannel "%(fromchannel)s" frompartner "%(frompartner)s" topartner "%(topartner)s".',
                                    {'route': self.ta_info['idroute'], 'fromchannel': self.ta_info['fromchannel'], 'frompartner': receiver, 'topartner': sender})
            # this info is used in transform.py to update the ta.....ugly...
            self.ta_info.update(confirmtype=confirmtype, confirmed=True,
                                confirmasked=True, confirmidta=ta_confirmation.idta)
