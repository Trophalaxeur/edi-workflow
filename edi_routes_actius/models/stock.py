...
class stock_picking(models.Model):
        ...
        if sale_order:
            if sale_order.partner_id.property_product_pricelist.id == 4:
                ...
                if self.partner_id.ref:
                    gln_no = self.partner_id.ref
                    route_id = self.partner_id.ref[-4:]
                builder.build_e1bpdlvpartner_element(header_element, 
                                                     '1', 
                                                     'AG', 
                ...
                gln_no = ''
                route_id = '0000'
                if sale_order.partner_id.ref:
                    gln_no = sale_order.partner_id.ref
                    route_id = sale_order.partner_id.ref[-4:]
                builder.build_e1bpdlvpartner_element(header_element, 
                                                     '1', 
                                                     'AG', 
    ...
    def edi_import_actius(self, document):
        ...
        delivery.number_of_pallets = 0
        delivery.number_of_packages = 0
        ...
        for edi_line in content['E1BPOBDLVITEMCON']:
            ...
            if edi_line['PALLETS']:
                delivery.number_of_pallets += int(edi_line['PALLETS'])
            if edi_line['COLLIS']:
                delivery.number_of_packages += int(edi_line['COLLIS'])

            processed_ids.append(pack_operation.id)

        # write number of unique pallets and packs to OUT
        #if len(list(set(crt_ids))) > 0:
        #    delivery.number_of_packages=len(list(set(crt_ids)))
        #if len(list(set(eur_ids))) > 0:
        #    delivery.number_of_pallets=len(list(set(eur_ids)))
        #if len(list(set(eur_ids))) > 0:
        #    delivery.list_of_pallets=str(list(set(eur_ids)))
