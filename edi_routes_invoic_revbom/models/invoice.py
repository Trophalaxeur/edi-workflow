        ...
        for line in invoice.invoice_line:
            ...
            if product.bom_ids and order.order_bomified:
                for bom in product.bom_ids[0].bom_line_ids:
                    ...
                    edi_line['AANTALGELEVERD'] = line.quantity*bom.product_qty
                    
                    for shadowproduct in order.order_shadow_line:
                        if shadowproduct.product_id.id == bomproduct.id:
                            edi_line['LIJNTOTAAL'] = shadowproduct.price_unit*bom.product_qty
                            edi_line['UNITPRIJS'] = shadowproduct.price_unit
                            edi_line['LIJNTOTAALBELAST'] = shadowproduct.price_unit*bom.product_qty

                    if partner.property_account_position.id in [1,4]:
                        for bom_tax in bomproduct.taxes_id:
                            vat = tax_db.browse(cr, uid, bom_tax.id, context)
